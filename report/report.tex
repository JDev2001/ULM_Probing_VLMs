...
\end{quote}
and the probe performs binary classification of whether the caption correctly describes the image. Due to limited computational resources, we just use one positive and one negative caption per image in contrast to \cite{tao2024probingmultimodallargelanguage}.
...
categories for each image. A set of 3 valid categories are sampled from each image for the category experiment
...
\end{figure}
In Figure \ref{fig:combined_metrics_per_layer}, we present a heatmap that summarizes the precision, recall, and F1 scores for the caption experiment across all layers and models.
...
For the category experiment on local semantics, the F1 score per layer across the two models is shown in Figure~\ref{fig:f1_per_layer_local}.
